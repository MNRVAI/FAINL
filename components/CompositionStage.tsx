  ...
  return (
    <div className="w-full space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
      <div className="text-center space-y-4">
        <div className="inline-flex items-center gap-2 px-4 py-1 bg-[var(--color-accent)] text-black text-[10px] font-black uppercase tracking-[0.3em] border-2 border-black">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {categories.map((cat) => (
          <div key={cat} className="flex flex-col gap-4">
            <h3 className="text-sm font-black uppercase tracking-[0.2em] text-[var(--color-accent)] text-center border-b-2 border-[var(--color-accent)] pb-2">
              {cat}
            </h3>
                ...
                return (
                  <button
                    ...
                    className={`group relative w-full text-left p-4 border-2 transition-all duration-300 ${
                      isSelected 
                        ? 'bg-black text-white border-black shadow-[6px_6px_0_0_var(--color-accent)]' 
                        : 'bg-white dark:bg-zinc-900 border-black/10 dark:border-white/10 hover:border-[var(--color-accent)] opacity-60 hover:opacity-100'
                    }`}
                  >
                    <div className="flex items-center gap-2 mb-2">
                      <img src={member.avatar} alt="" className="w-5 h-5 rounded-full object-cover border border-[var(--color-accent)]/50" />
                      <span className={`text-[9px] font-black uppercase tracking-widest ${isSelected ? 'text-[var(--color-accent)]' : 'text-black/40 dark:text-white/40'}`}>
                        {member.name}
                      </span>
                      {isSelected && <Check className="w-3 h-3 ml-auto text-[var(--color-accent)]" />}
                    </div>
                    ...
                    {!isSelected && (
                      <div className="absolute inset-0 bg-[var(--color-accent)]/5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <MousePointer2 className="w-6 h-6 text-[var(--color-accent)] animate-bounce" />
                      </div>
                    )}
                  </button>
                );
      ...
      {/* Preview & Submit */}
      <div className="bg-white dark:bg-black border-4 border-black dark:border-[var(--color-accent)] p-8 md:p-12 shadow-[12px_12px_0_0_black] dark:shadow-[12px_12px_0_0_var(--color-accent)] animate-in slide-in-from-bottom-4 duration-500">
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex-1 space-y-4">
            <h4 className="text-xl font-black uppercase tracking-widest text-[var(--color-accent)]">Jouw Samengestelde Antwoord</h4>
          ...
          <button
            ...
            className="shrink-0 flex items-center gap-4 px-10 py-6 bg-black text-white font-black text-xl uppercase tracking-widest hover:bg-[var(--color-accent)] hover:text-black transition-all hover:scale-105 active:scale-95 shadow-[8px_8px_0_0_var(--color-accent)] border-4 border-black"
          >
